...
//bool darkModeStatus = 1;  //put it into the header file, under the class now
...
const char *menuItems[] = {"Check Battery", "Vibrate Motor", "Show Accelerometer", "Set Time","Dark/Light Mode","Show Temperature","Stopwatch","Connect WiFi","BLE"};
...
void Watchy::syncNtpTime(){
    if(ntpSyncTimeCounter >= NTP_TIME_SYNC_INTERVAL){
        ...
        if (WiFi.status() == WL_CONNECTED) {    //or if(res)
...
void Watchy::showTemperature(){ //copied from other functions tbh
    display.init(0, false); //_initial_refresh to false to prevent full update on init
    display.setFullWindow();
    display.fillScreen(GxEPD_BLACK);
    display.setFont(&FreeMonoBold9pt7b);
    ...
    //int8_t temperature = RTC.temperature() / 4;
...
//testing
void Watchy::darkMode(uint8_t btnPin){ //copied from other functions tbh
    display.init(0, false); //_initial_refresh to false to prevent full update on init
    display.setFullWindow();
    display.fillScreen(GxEPD_BLACK);
    display.setFont(&FreeMonoBold9pt7b);
    ...
    display.setCursor(70, 80);
    if(btnPin == (DOWN_BTN_PIN | MENU_BTN_PIN)){    //toggle darkmode if button has been pressed
        darkModeStatus = !darkModeStatus;
    }
    display.println(darkModeStatus ? "On" : "Off");
    if(btnPin == (DOWN_BTN_PIN | MENU_BTN_PIN)){
